...
function navClass({ isActive }: { isActive: boolean }) {
  return `px-3 py-2 rounded-lg text-sm font-semibold ${
    ...
    <header className="border-b border-slate-800 bg-slate-950/70 backdrop-blur sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <nav className="ml-auto flex items-center gap-2">
          ...
          <NavLink to="/offerte" className={navClass}>
            Offerte
          </NavLink>

          {/* Link Admin visibile solo se admin loggato */}
          {role === 'admin' && (
          ...
          {!isAuthed ? (
            <>
              ...
              {/* Admin separato (questo risolve il tuo problema) */}
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg text-sm font-semibold bg-slate-800 hover:bg-slate-700"
            >
              Logout
            </button>
          )}
